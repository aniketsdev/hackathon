...
export default function Home() {
  ...
  const [githubAuth, setGithubAuth] = useState<GitHubAuthStatus>({
    connected: false,
    configured: false,
    user: null
  });
  ...
  const refreshGitHubAuth = useCallback(async () => {
    ...
    const status = (await response.json()) as GitHubAuthStatus;
    setGithubAuth(status);
  }, []);

  const disconnectGitHub = useCallback(async () => {
    await fetch("/api/auth/github/logout", { method: "POST" });
    setGithubAuth((current) => ({ connected: false, configured: current.configured, user: null }));
  }, []);
  ...
  const aiRiskMetric = useMemo(
    () => formatAiRiskMetric(result?.aiAnalysis, enableAiAnalysis),
  ...
  return (
    <main className={`app-shell app-${scoreTone}`}>
      <header className="topbar motion-in">
        ...
        <div className="top-actions">
          {githubAuth.connected ? (
            <button className="button button-secondary" onClick={disconnectGitHub} type="button">
              {githubAuth.user ? `GitHub: ${githubAuth.user}` : "GitHub connected"}
            </button>
          ) : (
      ...
      <section id="console" className="intake-grid motion-in" aria-label="Source intake">
      ...
      <section className="metrics-grid motion-in" aria-label="Scan overview">
        <MetricCard
          ...
          primary
        />
        <MetricCard label="AI risk" value={aiRiskMetric.value} detail={aiRiskMetric.detail} />
...
function summarizeAiStatus(aiAnalysis?: AIAnalysisResult): AiStatusView | null {
  ...
  if (aiAnalysis.status === "completed") {
    return {
      title: typeof aiAnalysis.riskScore === "number" ? `Risk ${aiAnalysis.riskScore}/100` : "Completed",
      tone: "done",
      lines: [
        aiAnalysis.riskLevel ? `AI severity: ${aiAnalysis.riskLevel}` : undefined,
        aiAnalysis.summary,
...
function formatAiRiskMetric(aiAnalysis: AIAnalysisResult | undefined, enabled: boolean) {
  if (!enabled) {
    return { value: "Off", detail: "Enable AI for risk scoring" };
  }

  if (!aiAnalysis || aiAnalysis.status === "skipped") {
    return { value: "-", detail: "Awaiting AI analysis" };
  }

  if (aiAnalysis.status !== "completed") {
    return { value: "Unavailable", detail: "Scanner score still applies" };
  }

  if (typeof aiAnalysis.riskScore !== "number") {
    return { value: "Ready", detail: "AI context generated" };
  }

  return {
    value: String(aiAnalysis.riskScore),
    detail: `${aiAnalysis.riskLevel ? `${capitalize(aiAnalysis.riskLevel)} risk` : "AI risk"} on a 0-100 scale`
  };
}
